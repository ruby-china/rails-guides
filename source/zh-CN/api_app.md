...
各个中间件的作用参见 [内部中间件栈](rails_on_rack.html#内部中间件栈)。
...
- `ActionController::Instrumentation`：提供 Action Controller 定义的监测钩子（详情参见 [Active Support 监测程序](active_support_instrumentation.html#Action Controller)）。
